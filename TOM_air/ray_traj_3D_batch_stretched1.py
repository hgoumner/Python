...
def N3D(pos):
    ...
    if type(pos) == np.ndarray:
        ...
        out = 1 + beta*Ts/t3d[idy,idx,idz]
        
        # Interpolation
        if xx in x and yy in y and zz in z:
            tint = spi.interpn((y,x,z),t3d,[yy,xx,zz],method='nearest')               
            out = 1 + beta*Ts/tint
        else:
            out = 1
...
ngy = n3dgy.ravel()
ngz = n3dgz.ravel()
ngtot = np.sqrt(ngx**2+ngy**2+ngz**2)
ngxp = abs(ngx/ngtot)
ngyp = abs(ngy/ngtot)
ngzp = abs(ngz/ngtot)
fo = np.c_[xo,yo,zo,to,tgx,tgy,tgz,no,ngx,ngy,ngz,ngtot,ngxp,ngyp,ngzp]
